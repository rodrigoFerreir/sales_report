...
def sales_data() -> List[Sale]:
    return [
        Sale("Laptop", Decimal("3500.50"), 2, date(2025, 10, 20)),
        Sale("Mouse", Decimal("150.00"), 5, date(2025, 10, 21)),
        Sale("Teclado", Decimal("300.25"), 3, date(2025, 10, 21)),
        Sale("Monitor", Decimal("800.00"), 1, date(2025, 10, 22)),
        Sale("Laptop", Decimal("3400.00"), 1, date(2025, 10, 22)),
        Sale("Fone de ouvido", Decimal("200.00"), 4, date(2025, 10, 23)),
        Sale("Mouse", Decimal("160.00"), 2, date(2025, 10, 23)),
    ]
...
def test_generate_report_no_filter(service, sales_data):
    ...
    assert report.total_per_product["Laptop"] == (
        Decimal("3500.50") * 2
    ) + (Decimal("3400.00") * 1)
    assert report.total_per_product["Mouse"] == (
        Decimal("150.00") * 5
    ) + (Decimal("160.00") * 2)
    assert report.total_per_product["Teclado"] == Decimal("300.25") * 3
    assert report.total_per_product["Monitor"] == Decimal("800.00") * 1
    assert report.total_per_product["Fone de ouvido"] == Decimal("200.00") * 4

    expected_grand_total = sum(report.total_per_product.values())
    assert report.value_total_sales == expected_grand_total
...
def test_generate_report_with_date_range_filter(service, sales_data):
    ...
    assert report.total_per_product["Mouse"] == Decimal("150.00") * 5
    assert report.total_per_product["Teclado"] == Decimal("300.25") * 3
    assert report.total_per_product["Monitor"] == Decimal("800.00") * 1
    assert report.total_per_product["Laptop"] == Decimal("3400.00") * 1
...
def test_generate_report_empty_sales(service):
    ...
    assert report.value_total_sales == Decimal("0.0")
    assert report.product_more_saled == []
...
def test_find_best_selling_product_tie(service):
    ...
    tied_sales = [
        Sale("A", Decimal("10"), 5, date(2025, 1, 1)),
        Sale("B", Decimal("10"), 5, date(2025, 1, 1)),
        Sale("C", Decimal("10"), 3, date(2025, 1, 1)),
    ]
    report = service.generate_report(tied_sales)
    assert isinstance(report, Report)
    assert report.product_more_saled == [("A", 5), ("B", 5)]
